...
public class Creative {
	...
	/** if this is a audio creative (NOT a native content video) its protocol */
	public Integer audioProtocol;
	/**
	 * if this is a audio creative (NOT a native content video) , the duration
	 * in seconds
	 */
	public Integer audioDuration;
	/** If this is a audio (Not a native content, the linearity */
	public Integer audioStartdelay;
	/** The audioMimeType */
	public String audioMimeType;
	//Audio Image Url
	public List<String> audioImageUrl;
	// Audio audioUrl
	public List<String> audioUrl;
	// click url
	public List<String> clickUrl;
	// Image tracking url
	public String imageTrackingUrl;
	// Audio tracking url
	public String audioTrackingUrl;
	// Audio agency_id
	public String agency_id;
	// Audio advertiser_name
	 ...
	 *
	 * @return boolean. Returns true if audio.
	 */
	@JsonIgnore
	...
	public SelectedCreative xproc(BidRequest br, String adId, Impression imp, Map<String, String> capSpecs, StringBuilder errorString, Probe probe) {
		...
		System.out.println("audioDuration "+ this.audioDuration);
		System.out.println("videoDuration "+ this.videoDuration);
		if (br.checkNonStandard(this, errorString) != true) {
			return null;
		}
		...
		if (isAudio() && imp.audio == null) {
			probe.process(br.getExchange(), adId, impid, Probe.BID_CREAT_IS_AUDIO);
			if (errorString != null)
		...
		if ((isAudio() == false && isNative() == false) != (imp.nativePart == null && imp.audio == null)) {
			probe.process(br.getExchange(), adId, impid, Probe.BID_CREAT_IS_BANNER);
			if (errorString != null)
				errorString.append(Probe.BID_CREAT_IS_BANNER);
			return null;
		 ...
		 */
		if (imp.audio != null) {
			if (imp.audio.startdelay != -1 && this.audioStartdelay != null) {
				if (imp.audio.startdelay != this.audioStartdelay) {
					probe.process(br.getExchange(), adId, impid, Probe.AUDIO_STARTDELAY);
					if (errorString != null)
						errorString.append(Probe.AUDIO_STARTDELAY);
					return null;
				}
			}
			if (imp.audio.minduration != -1) {
				if (this.audioDuration != null) {
					if (!(this.audioDuration.intValue() >= imp.audio.minduration)) {
						probe.process(br.getExchange(), adId, impid, Probe.AUDIO_TOO_SHORT);
						if (errorString != null)
							errorString.append(Probe.AUDIO_TOO_SHORT);
						return null;
					}
				}
			}
			if (imp.audio.maxduration != -1) {
				if (this.audioDuration != null) {
					if (!(this.audioDuration.intValue() <= imp.audio.maxduration)) {
						probe.process(br.getExchange(), adId, impid, Probe.AUDIO_TOO_LONG);
						if (errorString != null)
							errorString.append(Probe.AUDIO_TOO_LONG);
						return null;
					}
				}
			}
			if (imp.audio.protocol.size() != 0) {
				if (this.audioProtocol != null) {
					if (imp.audio.protocol.contains(this.audioProtocol) == false) {
						probe.process(br.getExchange(), adId, impid, Probe.AUDIO_PROTOCOL);
						if (errorString != null)
							errorString.append(Probe.AUDIO_PROTOCOL);
						return null;
					}
				}
			}
			if (imp.audio.mimeTypes.size() != 0) {
				if (this.audioMimeType != null) {
					if (imp.audio.mimeTypes.contains(this.audioMimeType) == false) {
						probe.process(br.getExchange(), adId, impid, Probe.AUDIO_MIME);
						if (errorString != null)
							errorString.append(Probe.AUDIO_MIME);
						return null;
					}
				}
			}
