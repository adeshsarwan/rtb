...
public class BidResponse {
	...
	public void makeResponse(double price) throws Exception {
		...
		if (imp.video != null)
			this.adtype="video";
		else if (imp.audio != null)
			this.adtype="audio";
		else
			this.adtype="banner";
		/******************************************/

		if (this.creat.isAudio()) {
			AudioBidResponse audioBidResponse = new AudioBidResponse(this);
			response = audioBidResponse.generateAudioBidResponse();
		} else {
			/** The configuration used for generating this response */
			Configuration config = Configuration.getInstance();
			StringBuilder nurl = new StringBuilder();
